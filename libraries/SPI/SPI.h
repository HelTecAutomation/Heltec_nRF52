...
class SPIClass {
  ...
  private:
    ...
    SemaphoreHandle_t _mutex_spi=NULL;

    void _mutex_lock  (void) { do {} while (xSemaphoreTake(_mutex_spi, portMAX_DELAY) != pdPASS); }
    void _mutex_unlock(void) { xSemaphoreGive(_mutex_spi); }
